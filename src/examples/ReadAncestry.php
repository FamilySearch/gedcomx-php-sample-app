...
<?php
  ...
  if($personId){
    ...
    // No errors
      ...
      if($ancestryResponse->hasError()){
        ...
        ?>
          <h2>Iterative</h2>
          <p>This list of ancestors was generated by iterating over the Ahnentafel
          numbers. It should match the recursive list above.</p>
          <table class="table">
            <tr>
              <th>Ahnentafel #</th>
              <th>ID</th>
  ...
  function printTreeNodeRecursive($node){
    if($node){
      printTreeNode($node);
      printTreeNodeRecursive($node->getFather());
      printTreeNodeRecursive($node->getMother());
    }
  }
